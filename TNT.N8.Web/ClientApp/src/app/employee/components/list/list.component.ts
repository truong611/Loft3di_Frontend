...
import { GiaDinhImportDetailComponent } from '../employee-profile/employee-details/thong-tin-gia-dinhs/gia-dinh-import-detail/gia-dinh-import-detail.component';
...
export class ListComponent implements OnInit {
  ...
  importHopDong() {
    ...
    reader.onload = (e: any) => {
      ...
      let columns = [
        ...
        { field: 'SoPhuLuc', type: 'text', isRequired: false },
        { field: 'NgayKyHD', type: 'date', isRequired: true },
        { field: 'NgayBDLamViec', type: 'date', isRequired: true },
        { field: 'NgayKetThucHD', type: 'date', isRequired: false },
      ...
      listTaiSanRawData?.forEach(_rawData => {
        ...
        columns.forEach((item, index) => {
          ...
          if (item.type == 'date') customer[item.field] = _rawData[index] ? my_date(_rawData[index]) : null;
        });
        listTaiSanRawImport = [...listTaiSanRawImport, customer];
      });
      /* tắt dialog import file, bật dialog chi tiết khách hàng import */
  ...
  async downloadTemplateExcelHopDong(number: Number) {
    // 1: Hợp đồng
    // 2: Gia đình
    this.loading = true;
    let result: any = await this.employeeService.downloadTemplateImportHDNS(number);
    this.loading = false;
    if (result.templateExcel != null && result.statusCode === 200) {
      const binaryString = window.atob(result.templateExcel);
      const binaryLen = binaryString.length;
...
function my_date(date_string) {
  ...
  if (typeof (date_string) === typeof (new Date())) {
    let day = new Date(date_string.toString().trim()).getDay()
    let month = new Date(date_string.toString().trim()).getMonth()
    let year = new Date(date_string.toString().trim()).getFullYear()
    return new Date(year, month, day);

  } else {
    var date_components = date_string.toString().trim().split("/");
