...
@Component({
  selector: 'app-de-xuat-tang-luong-list',
  templateUrl: './de-xuat-tang-luong-list.component.html',
  styleUrls: ['./de-xuat-tang-luong-list.component.css']
})

export class DeXuatTangLuongListComponent implements OnInit {
  ...
  goToCreate() {
    this.router.navigate(['/employee/tao-de-xuat-tang-luong']);
  }

  async ngOnInit() {
    this.initTable();
    let resource = "hrm/employee/danh-sach-de-xuat-tang-luong/";
  ...
  initTable() {
    this.colsList = [
      ...
      { field: 'tongMucDeXuat', header: 'Tổng mức đề xuất', textAlign: 'left', display: 'table-cell' },
    ];
  ...
  searchDeXuatTangLuong() {
    ...
    this.loading = true;
    this.employeeListService.listDeXuatTangLuong(nguoiDeXuat, tgianDeXuat, trangThai).subscribe(response => {
      let result = <any>response;
      this.loading = false;
      if (result.statusCode == 200) {
        this.listDeXuatTangLuong = result.listDeXuatTangLuong;
        if (this.listDeXuatTangLuong.length === 0) {
          let msg = { severity: 'warn', summary: 'Thông báo:', detail: 'Không tìm thấy đề xuất tăng lương nào!' };
          this.showMessage(msg);
        }
      } else {
  ...
  deleteDeXuat(rowData: any) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.loading = true;
        this.employeeListService.deleteDeXuatTangLuong(rowData.deXuatTangLuongId).subscribe(response => {
          let result = <any>response;
          this.loading = false;
          if (result.statusCode == 200) {
            let msg = { severity: 'success', summary: 'Thông báo:', detail: result.message };
            this.listDeXuatTangLuong = this.listDeXuatTangLuong.filter(item => item.deXuatTangLuongId != rowData.deXuatTangLuongId);
            this.showMessage(msg);
          } else {
  ...
  setIndex() {
    this.listDeXuatTangLuong.forEach((item, index) => {
      item.index = index + 1;
    });
  }
  onViewDetail(rowData: any) {
    this.router.navigate(['/employee/de-xuat-tang-luong-detail', { deXuatTLId: this.encrDecrService.set(rowData.deXuatTangLuongId) }]);
  }
